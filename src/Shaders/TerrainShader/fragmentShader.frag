...
{ 
    ...
    m = m * m;
    return 10.5 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), 
                                    dot(p2,x2), dot(p3,x3) ) );
}
...
void main(void)
{
    ...
    if (ambientColor == vec3(1,1,1)) {
        ...
        if (steepNess > 0.8) {
            ...
            color = vec3(0.53, 0.32, 0.22);
        }
        out_Pixel = vec4(diffuseColor, 1.0) * vec4(color, 1.0);
    } else {
        out_Pixel = vec4(diffuseColor, 1.0) * vec4(ambientColor, 1.0);
    }
}
